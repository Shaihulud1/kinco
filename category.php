...
?>
<?php 
// SELECT fi.fname, fi.fposter, ca.cname FROM films_categories fc
//                             INNER JOIN films fi ON fc.id_film = fi.id
//                             INNER JOIN categorys ca ON fc.id_cat = ca.id
//               WHERE ca.cname = 'Триллер'
...
////////////////
	...
	$sql = ("SELECT fc.id, fi.fname, fi.fposter, ca.cname FROM films_categories fc 
                            INNER JOIN films fi ON fc.id_film = fi.id
                            INNER JOIN categorys ca ON fc.id_cat = ca.id  
              WHERE ca.cname = '".$_GET['n']."' ORDER BY fc.id DESC LIMIT $start, $num " );
    		$result = $pdo->prepare($sql);
    		$result ->execute();
...
<?php
  ...
  ///вывод пагинации///
    ...
    if ($page != 1) $pervpage = '<a href=category.php?n='.$_GET['n'].'&page=1>Первая</a> | <a href=category.php?n='.$_GET['n'].'&page='.($page-1).'>Предыдущая</a> | ';
    //нужны ли стрелки вперед
    if ($page != $total) $nextpage = ' | <a href=category.php?n='.$_GET['n'].'&page='.($page + 1). '>Следующая</a> | <a href=category.php?n='.$_GET['n'].'&page='.$total.'>Последняя</a>';


    if($page - 5 > 0) $page5left = '<a href=category.php?n='.$_GET['n'].'&page='.($page-5).'>'.($page-5).'</a> | ';
    if($page - 4 > 0) $page4left = '<a href=category.php?n='.$_GET['n'].'&page='.($page-4).'>'.($page-4).'</a> | ';
    if($page - 3 > 0) $page3left = '<a href=category.php?n='.$_GET['n'].'&page='.($page-3).'>'.($page-3).'</a> | ';
    if($page - 2 > 0) $page2left = '<a href=category.php?n='.$_GET['n'].'&page='.($page-2).'>'.($page-2).'</a> | ';
    if($page - 1 > 0) $page1left = '<a href=category.php?n='.$_GET['n'].'&page='.($page-1).'>'.($page-1).'</a> | ';

    if($page + 5 <= $total) $page5right = ' | <a href=category.php?n='.$_GET['n'].'&page='.($page+5).'>'.($page+5).'</a>';
    if($page + 4 <= $total) $page4right = ' | <a href=category.php?n='.$_GET['n'].'&page='.($page+4).'>'.($page+4).'</a>';
    if($page + 3 <= $total) $page3right = ' | <a href=category.php?n='.$_GET['n'].'&page='.($page+3).'>'.($page+3).'</a>';
    if($page + 2 <= $total) $page2right = ' | <a href=category.php?n='.$_GET['n'].'&page='.($page+2).'>'.($page+2).'</a>';
    if($page + 1 <= $total) $page1right = ' | <a href=category.php?n='.$_GET['n'].'&page='.($page+1).'>'.($page+1).'</a>';
